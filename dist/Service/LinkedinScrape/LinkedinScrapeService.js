...
class LinkedInScraperService {
    ...
    async postJobFirstForm(job_title, contract_type, description, questions) {
        try {
            ...
            await this.page.evaluate(() => {
                Array.from(document.querySelectorAll("button .artdeco-button__text")).forEach((submitButton) => {
    ...
    async goToQuestionSection(questions) {
        if (!this.page)
            throw new Error('Browser has not been launched.');
        ...
        await this.page.evaluate((questions) => {
            console.log("JERE 4");
            Array.from(document.querySelectorAll(".artdeco-button.artdeco-button--muted")).forEach(async (submitButton) => {
                if (submitButton.textContent === "        \n    \n    \n\n\n\n\n    Custom Question\n") {
...
exports.default = LinkedInScraperService;
